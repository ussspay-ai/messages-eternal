        ...
        try {
            ...
            timeout = setTimeout(() => {
                ...
                if (connectionEstablished) {
                    addResult('SSE Connection', true, `Connection established and maintained for 10 seconds. Received: ${heartbeatCount} heartbeats, ${messageCount} messages`);
                    log('success', `Connection test passed! Heartbeats: ${heartbeatCount}, Messages: ${messageCount}`);
                }
                else {
                    addResult('SSE Connection', false, 'Connection timeout - no connection established');
...
async function testMessageGeneration() {
    ...
    try {
        ...
        if (data.success && data.messagesGenerated > 0) {
            addResult('Message Generation', true, `Generated ${data.messagesGenerated} messages, broadcasted to SSE subscribers`);
...
async function printSummary() {
    ...
    console.log(`${colors.bright}Results: ${colors.green}${passed} passed${colors.reset}, ${colors.red}${failed} failed${colors.reset}${colors.bright}\n${colors.reset}`);
    return failed === 0;
}
async function main() {
    console.log(`
