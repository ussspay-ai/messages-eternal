 ...
 * 
 * Usage: npx ts-node test-chat-system.ts
 * 
 * Tests:
...
interface DiagnosticResult {
  ...
  agentCredentials: Record<string, boolean>
  positionsFetch: Record<string, any>
  marketPrices: any
  summary: any
  errors: string[]
}
...
async function runDiagnostics() {
  ...
  try {
    ...
    const diagnostics: DiagnosticResult = await response.json()
    ...
    console.log("\n📍 POSITION FETCHES:")
    Object.entries(diagnostics.positionsFetch).forEach(([agent, data]: [string, any]) => {
