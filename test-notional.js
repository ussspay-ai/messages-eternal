  ...
  try {
    ...
    for (const agent of agents) {
      ...
      if (!posRes.ok) {
        ...
        console.log(`    Response: ${await posRes.text()}`)
        continue
      }
      ...
      if (positions.length > 0) {
        console.log(`   Raw data structure:`, JSON.stringify(positions[0], null, 2))
      }
      ...
      if (!exitRes.ok) {
        ...
        console.log(`    Response: ${await exitRes.text()}`)
        continue
      }
      const exitData = await exitRes.json()
      const exitPlans = Array.isArray(exitData) ? exitData : (exitData.exitPlans || [])
      ...
      for (const pos of positions) {
        ...
        const exitPlan = exitPlans.find((ep) => ep.coin === coin)?.exitPlan || "N/A"
